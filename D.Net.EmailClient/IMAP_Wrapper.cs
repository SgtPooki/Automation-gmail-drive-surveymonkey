...
namespace D.Net.EmailClient
{
    public class IMAP_Wrapper : IEmailClient
    {
        ...
        public IMAP_t_SeqSet getEmailsByFlag(String searchCriteria)
        {
            //Client.
            int[] unseen_ids = Client.Search(false, "UTF-8", searchCriteria);
            int count = unseen_ids.Length;

            if (count > 0)
            {
                //unSeenlist = new List<string>();

                //Console.WriteLine("");

                //IMAP_t_SeqSet seqSet = new IMAP_t_SeqSet();

                IMAP_t_SeqSet.Parse("1:*");

                //IMAP_t_SeqSet sequence = new IMAP_t_SeqSet();

                // now we need to initiate our sequence of messages to be fetched
                //IMAP_t_SeqSet.Parse(string.Join(",", unseen_ids));

                return IMAP_t_SeqSet.Parse(string.Join(",", unseen_ids)); ;
            }
            else
            {
                //IMAP_t_SeqSet seqSet = new IMAP_t_SeqSet();
                return IMAP_t_SeqSet.Parse("999999");
                //return seqSet;
            }

        }

        public IMAP_t_SeqSet getEmailByUID(String UID)
        {
            //Client.
            int[] unseen_ids = Client.Search(true, "UTF-8", UID);
            int count = unseen_ids.Length;
            ...
            {
                return IMAP_t_SeqSet.Parse(UID + ":" + UID);
            }
        }
        ...
        public void LoadMessages(String searchCriteria, String end)
        {
            if (!_IsConnected) throw new EMailException { ExceptionType = EMAIL_EXCEPTION_TYPE.NOT_CONNECTED };
            if (!String.IsNullOrWhiteSpace(_CurrentFolder))
            {
                Client.Fetch(
                    ...
                    getEmailsByFlag(searchCriteria),
                    new IMAP_t_Fetch_i[] {
        ...
        public void LoadMessages(String start, String end, bool useUIDs)
        {
            this.LoadMessages(start, end);
        }

        public void LoadMessages(String searchCriteria)
        {
            this.LoadMessages(searchCriteria, "*");
        }
    ...
    public class IMAP_Message_Wrapper: IEmail
    {
        ...
        public void SetUnread()
        {
            ...
            Client.StoreMessageFlags(true, seq, IMAP_Flags_SetType.Remove, new IMAP_t_MsgFlags(new string[] { IMAP_t_MsgFlags.Seen }));
        }
    }
